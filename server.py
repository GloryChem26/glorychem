"""
GloryChem — Python Web Server with Socket.IO (asyncio / ASGI version)
Run locally:  uvicorn server:asgi_app --host 0.0.0.0 --port 5000 --reload
Deploy Render: uvicorn server:asgi_app --host 0.0.0.0 --port $PORT --workers 1
"""
...
socketio = SocketIO(
    ...
    async_mode="threading",   # works with uvicorn/gunicorn threads, no monkey-patch needed
    ping_timeout=60,
    ping_interval=25,
...
#   IN-MEMORY STATE
# ══════════════════════════════════════
queued_players: list   = []   # list of (sid, user_data)
game_rooms:     dict   = {}   # room_id -> RoomState dict
online_users:   dict   = {}   # sid -> user_data
sid_to_uid:     dict   = {}   # sid -> userId
uid_to_sid:     dict   = {}   # userId -> sid
pending_invites: dict  = {}   # room_id -> invite metadata
...
def cancel_timer(room, key):
    """Cancel a stored asyncio.TimerHandle or asyncio.Task."""
    h = room.pop(key, None)
    if h is None:
        return
    try:
...
def _schedule(delay: float, fn, *args):
    """
    Schedule `fn(*args)` after `delay` seconds using the running event loop
    (falls back to threading.Timer when no loop is available, e.g. during tests).
    ...
    try:
        ...
        pass
    # Fallback: threading.Timer (safe for Flask threading mode)
    import threading
...
def end_battle_and_update(room_id):
    ...
    user1_id, user2_id = u1.get("userId"), u2.get("userId")
    ...
    p1_db, p2_db = profiles.get(user1_id, {}), profiles.get(user2_id, {})

    elo1, elo2 = p1_db.get("elo", 1200), p2_db.get("elo", 1200)
    ...
    socketio.emit(
        "battle_result",
        {
            ...
            "elo_change":   {user1_id: upd1["elo"] - elo1, user2_id: upd2["elo"] - elo2},
...
def handle_invite_friend(data):
    ...
    pending_invites[room_id] = {"from_sid": sid, "to_sid": to_sid}

    socketio.emit(
        "invite_received",
        {"roomId": room_id, "from": make_user_payload(from_user)},
        room=to_sid,
    )
...
def handle_invite_response(data):
    ...
    socketio.emit(
        "match_found",
        {"roomId": room_id, "role": "host", "opponent": make_user_payload(to_user)},
        room=from_sid,
    )
    socketio.emit(
        "match_found",
        {"roomId": room_id, "role": "guest", "opponent": make_user_payload(from_user)},
        room=sid,
    )
...
def _send_current_question(room_id):
    ...
    def _on_q_timeout():
        ...
        if (
            r
            and r["state"] == "battle"
            and r["current_question"] == q_idx
            and not r["question_finished"]
        ):
...
def _end_battle(room_id):
    ...
    _schedule(10, lambda: game_rooms.pop(room_id, None))
...
if __name__ == "__main__":
    ...
    # Chạy local với flask-socketio threading mode

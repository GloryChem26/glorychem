...
class Game {
    constructor() {
        ...
        this.saltStorage = {};
        this.fastElectroUnlocked = false;
    ...
    initElectroPanel() {
        ...
        document.getElementById('lab-btn').addEventListener('click', () => {
            ...
            // Cập nhật tiêu đề panel nếu đã mở khóa x2
            const panelTitle = document.querySelector('.mc-title');
            if (panelTitle && this.fastElectroUnlocked) {
                panelTitle.innerHTML = '⚡⚡ Bàn Điện Phân <span style="color:#ffe066;font-size:12px;margin-left:6px">[x2 NHANH]</span>';
            }
        });
...
Cần ${recipe.requires} để điện phân`;
    ...
    _startSmelt() {
        ...
        // Nếu đã mở khóa điện phân nhanh x2 → giảm nửa thời gian
        const speedMultiplier = this.fastElectroUnlocked ? 0.5 : 1;
        const duration = Math.min(10000, 1000 * this._ep.inputSalt.multiples) * speedMultiplier;
    ...
    _refreshEpInventory() {
        ...
        // Cập nhật nút smelt nếu đã mở khóa điện phân nhanh
        const smeltBtn = document.getElementById('mc-smelt-btn');
        if (smeltBtn && this.fastElectroUnlocked) {
            smeltBtn.textContent = '⚡⚡ Điện Phân Nhanh x2!';
            smeltBtn.style.background = 'linear-gradient(135deg, #ffe066, #ff9900)';
            smeltBtn.style.color = '#111';
        }
    }

    _showUnlockBanner() {
        // Tạo banner popup thông báo mở khóa
        const banner = document.createElement('div');
        banner.id = 'unlock-banner';
        banner.innerHTML = `
            <div class="unlock-icon">⚡⚡</div>
            <div class="unlock-text">
                <div class="unlock-title">KỸ NĂNG MỞ KHÓA!</div>
                <div class="unlock-desc">Điện Phân Nhanh x2</div>
                <div class="unlock-sub">Tốc độ điện phân tăng gấp đôi từ Wave 6 trở đi</div>
            </div>
        `;
        banner.style.cssText = `
            ...
            background: linear-gradient(135deg, rgba(20,10,0,0.97), rgba(60,40,0,0.97));
            ...
            align-items: center;
            gap: 20px;
            box-shadow: 0 0 60px rgba(255,220,50,0.6), 0 0 120px rgba(255,150,0,0.3);
            transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1);
            pointer-events: none;
        `;
        banner.querySelector('.unlock-icon').style.cssText = `font-size: 52px; line-height: 1; text-shadow: 0 0 20px #ffe066;`;
        banner.querySelector('.unlock-text').style.cssText = `text-align: left;`;
        banner.querySelector('.unlock-title').style.cssText = `font-size: 13px; color: #ffe066; letter-spacing: 3px; font-weight: 800; text-transform: uppercase;`;
        banner.querySelector('.unlock-desc').style.cssText = `font-size: 26px; color: #fff; font-weight: 900; margin: 4px 0;`;
        banner.querySelector('.unlock-sub').style.cssText = `font-size: 12px; color: #ffcc77;`;
        ...
        // Cập nhật header bàn điện phân
        const labLabel = document.querySelector('.lab-label');
        if (labLabel) labLabel.textContent = 'Điện Phân x2 ⚡⚡';
    }

    async loadAssets() {
        const imgMap = {
            acidplant: 'game/GloryDefense/images/acidplant.png',
            baseplant: 'game/GloryDefense/images/baseplant.png',
            waterplant: 'game/GloryDefense/images/waterplant.png',
            ground: 'game/GloryDefense/images/ground.png',
            zombie1: 'game/GloryDefense/images/z1-sheet0.png',
            zombie2: 'game/GloryDefense/images/z2-sheet0.png',
            zombie3: 'game/GloryDefense/images/z3-sheet0.png',
            protect: 'game/GloryDefense/images/protect.png',
            shovel: 'game/GloryDefense/images/xeng.png'
        };
    ...
    update(dt) {
        ...
        if (this.waveTimer <= 0) {
            ...
            // Mở khóa điện phân x2 sau wave 5
            if (this.wave > 5 && !this.fastElectroUnlocked) {
                this.fastElectroUnlocked = true;
                this.showReaction('⚡🔓 MỞ KHÓA: Điện Phân Nhanh x2! Tốc độ điện phân tăng gấp đôi!', true);
